...
1. 6단계에서 코드를 GitHub에 올렸다면 **자동 배포가 이미 돌고 있습니다.** 그대로 1~2분 기다립니다.
2. 사이트 주소를 새로고침합니다.
3. 3단계에서 넣은 데이터가 **화면에 목록으로** 보이면 성공입니다.

> **코드를 안 바꾸고 환경변수만 고친 경우**(예: 값을 잘못 넣어 다시 저장)는 자동 배포가 안 돕니다. Vercel **Deployments → ⋯ → Redeploy**로 직접 재배포하세요.
...
| 순서 | 증상 | 원인 | 해결 |
|------|------|------|------|
| 1 | 값을 넣었는데 화면이 그대로 | **재배포 안 함** — 가장 흔함 | 7단계 재배포. 배포가 끝났는지부터 확인 |
| 2 | 화면에 아무것도 안 나옴 | 환경변수 **철자** 오타 | `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` 이름을 1:1로 대조 |
| 3 | 여전히 빈 목록 (에러는 없음) | **RLS(행 보안)**가 켜져 있는데 공개 조회 정책 없음 | 아래 "읽기 공개 정책" 참고 (또는 강사에게 요청) |
| 4 | 여전히 빈 화면 | 표 이름 / 열 이름 **불일치** | 코드의 `from('...')`·`n.열이름`이 Supabase 표·열 이름과 정확히 같은지 확인 |
| 5 | `permission denied` 류 오류 | RLS 정책이 조회를 막고 있음 | 위 3번과 동일 |
| 6 | 로컬은 되는데 배포만 실패 | 환경변수가 **Vercel에만 없음** | Vercel Settings에 같은 값이 있는지 확인 후 재배포 |

### 읽기 공개 정책 (RLS) — 3·5번 증상일 때
...
- **가장 자주 막히는 지점** 순서 고정: (1) 재배포 누락 → (2) 환경변수 철자 → (3) RLS 공개 정책 → (4) 표/열 이름 일치. 체크리스트를 이 순서로 짚으면 대부분 잡힙니다.
- **`anon` 키가 "공개해도 되는 키"인 이유는 RLS입니다.** 이 키는 사이트에 실려 브라우저까지 내려갑니다. RLS를 끄면 그 보호막이 없어져 읽기뿐 아니라 쓰기·삭제까지 열리므로, 이 표에는 실습용 데이터만 두게 안내하세요. 6교시(로그인·글쓰기)까지 가는 팀은 RLS를 켠 채 읽기 정책을 거는 쪽으로 진행해야 합니다.
