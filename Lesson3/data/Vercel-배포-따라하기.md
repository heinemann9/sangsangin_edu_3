...
- **강사가 알려준 튜토리얼 저장소 주소** (1단계에서 복사할 원본)
- **이메일** (Vercel 가입용 — GitHub로 바로 가입 가능)
- 웹 브라우저 (크롬 권장)
- **Claude Code** — 1단계의 저장소 복사(미러링) 한 곳에서만 씁니다. 나머지는 전부 웹 화면입니다.

전체 흐름은 이렇게 8단계입니다.

```
0. Vercel 로그인  →  1. 내 저장소 만들기(미러링)  →  2. GitHub 연결·설치
   →  3. 프로젝트 가져오기(Import)  →  4. 배포(Deploy)  →  5. 사이트 확인
   →  6. 환경변수 넣기  →  7. 다시 배포  →  8. 결과 확인
```

> **저장소를 먼저 만드는 이유**: 2단계에서 Vercel에 GitHub를 연결할 때, 가져올 저장소가 **이미 있어야** 접근 권한을 그 저장소에 바로 줄 수 있습니다. 순서가 반대면 빈 목록을 보고 다시 돌아와야 합니다.
...
1. 저장소 목록(또는 검색)에서 `vercel-test`를 찾아 **Import**(가져오기)를 누릅니다. 안 보이면 2단계의 접근 권한에 그 저장소가 빠진 것입니다.
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 저장소 목록에 우리 저장소가 안 보임 | Vercel 앱이 그 계정/조직에 설치 안 됨 | 2단계처럼 해당 계정에 Vercel 설치 (All 또는 해당 저장소 선택) |
...
- 2단계(GitHub 연결·설치)가 실제 현장에서 가장 자주 막히는 지점입니다. 계정/조직 선택과 저장소 접근 범위를 화면 공유로 함께 짚어주세요.
- **미러링(1단계)을 GitHub 연결(2단계)보다 앞에 두었습니다.** 저장소가 먼저 있어야 권한 부여 화면에서 `vercel-test`를 바로 고를 수 있고, Import 화면에 두 번 들어가는 왕복이 사라집니다. MCP 버전 문서도 같은 순서입니다.
