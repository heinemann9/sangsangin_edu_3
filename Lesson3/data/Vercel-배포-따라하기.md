...
- **GitHub 계정** + 우리 팀 프로젝트가 올라간 저장소 (1·2회차에서 준비 완료)
...
```
0. Vercel 로그인  →  1. GitHub 연결·설치  →  2. 프로젝트 가져오기(Import)
   →  3. 배포(Deploy)  →  4. 사이트 확인  →  5. 환경변수 넣기  →  6. 다시 배포  →  7. 결과 확인
```
...
| 환경변수 넣었는데 화면이 그대로 | 재배포를 안 함 | 6단계 재배포 |
...
- 핵심 체크포인트: **3단계(배포 주소 확보)** 와 **7단계(환경변수 반영 확인)**. 막힌 팀은 이 두 지점을 기준으로 따라잡힙니다.
